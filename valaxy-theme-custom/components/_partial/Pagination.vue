...
<template>
    <div class="mx-auto max-w-sm mb-4">
        <nav role="navigation" aria-label="pagination">
            <div class="buttons gap-2 flex justify-between">
                <ul>
                    <li v-if="curPage !== 1">
                        <RouterLink :to="jumpTo(curPage - 1)" class="button" aria-label="To previous page">
                ...
                <ul class="hidden md:contents" v-for="i in totalPages">
                    <li v-if="showPage(i)" :key="i">
                        <RouterLink aria-label="To page" :to="jumpTo(i)" class="button"
                            :class="curPage === i && 'is-link'">{{ i
                            }}</RouterLink>
                    </li>
                    <li v-else-if="i === curPage - surLen" class="mx-4" :key="`prev-space-${i}`">
                        ...
                    </li>
                    <li v-else-if="i === curPage + surLen" class="mx-4" :key="`next-space-${i}`">
                        ...
                    </li>
                ...
                <ul>
                    <li v-if="curPage !== totalPages">
                        <RouterLink :to="jumpTo(curPage + 1)" class="button" aria-label="To next page">
